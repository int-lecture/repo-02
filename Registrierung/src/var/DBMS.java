...
public class DBMS {
	...
	public synchronized long retrieveAndUpdateSequence(String userId) {
		...
		if (seqDoc != null) {
			...
			sequences.updateOne(eq("user", seqDoc.get("user")), new Document("$set", seqDoc));
		} else {
			sequences.insertOne(new Document("sequence", sequence).append("user", userId));
		}
	 ...
	 *      long, boolean)
	 */
	public synchronized List<Message> retrieveMessages(String userId, long sequenceNumber, boolean deleteOldMessages) {
